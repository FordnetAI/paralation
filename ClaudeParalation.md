...
### ART DIRECTION v2 (locked Aug 16 2026): Ghibli painterly, not pixel art
- Tim called the pivot: away from pixel art, toward the casual-game cartoon family. Bake-off across 6 candidates (Z prompt-only, Casual Game Art, Ghibli, Clay, Cute 3D, + gated ones) on 4 fixed-seed probes. GHIBLI WON on workability: 0.00-0.04% backdrop leak and perfect single-object discipline, where Casual Game Art baked ground slabs and scenery into half its probes (it is a scene illustrator, not an asset generator). Full evidence: shots history, workability-sbg-vs-ghibli sheet.
- Stack: Flux Dev fp8 checkpoint + Ghibli_style_flux.safetensors LoRA (18MB, trigger "Ghibli style") at 0.95, FluxGuidance 3.5, euler/simple, 24 steps, cfg 1. ~35s/image on the 4070 Ti Super. make_workflows.py is the source of truth; all five paralation-* workflows regenerated and installed on this stack.
- Importer: keying + comic outline + anchoring UNCHANGED; the pixel steps are gone (LANCZOS instead of BOX, soft alpha with a <24 haze floor instead of hard threshold, no 32-colour quantize). pixelize() keeps its name so callers did not change.
- Engine: canvas CSS no longer image-rendering:pixelated. Code-drawn art (Matt, floors, walls, yard fallbacks) is still pixel art and now reads mismatched next to painterly sprites ON PURPOSE for the interim: floors/walls were already art phase 2, and Matt needs a Ghibli redesign pass.
- Division of labour after the popup incident: CLAUDE NEVER launches, restarts, or queues Comfy Desktop; Tim runs all generation in the app himself. Claude writes workflows/files, reads output folders, imports.
- The live site stays on the last pixel build until the world regen lands, so the public game never shows a half-converted state.
- Six more style LoRAs are login-gated on CivitAI (list + exact target filenames in the styletest section of the chat log / paralation-styletest-b workflow); Tim may still download them to extend the bake-off. paralation-styletest-a/b workflows remain installed for that.

### The village + school (workflows 6-8, built Aug 16 2026, not yet generated or mapped)
- Tim's brief redefined the "main street" as a BRITISH COUNTRYSIDE village: pubs The Bell and The Goat, S&M Stores, The Other Shop, a butcher, an Indian restaurant, a post office, three green spaces (small park w/ swings; medium park w/ football pitch, swings, see-saw, roundabout; large sports ground w/ 2 pitches, changing rooms, sports & social club), plus a modular secondary school (main building, sports hall, gates) and grounds/street/story kits. Shop names are personal - never "fix" them.
- SETTING COLLISION, raised with Tim. His call (Aug 16 2026): "we will pretend its a village a little out. worry about story later, right now look and feel." So: the village sits "a little out" from Matt's neighbourhood, the San Diego story text stays AS IS for now, and no story lines get edited until Tim reopens the question. Do not resolve this unilaterally in either direction.
- Kits were pre-anglicized to match the brief rather than the original American list: pillar box not hydrant, covered bike shed not rack, small spectator stand not bleachers, white minibus coach not yellow school bus. Football pitch LINE MARKINGS are not an asset; they belong to ground tiles / map work later.
- SIZES entries for all 27 new assets are PROVISIONAL until the village and school maps are laid out.

### Art phase 2+3 tooling (workflows 9-11, built Aug 16 2026, awaiting generation)
- paralation-9-ground: 12 seamless textures (6 floors/grounds, 6 walls). New 'tex' view in full_prompt (no magenta, no object framing). Importer path: square-crop, 256px, edge crossfade toward the opposite edge (TEX_BLEND=20) so tiling wraps. Engine: SPR.TEX maps grid chars to textures, texSlice() samples 32px per tile at world coords, and EVERY char falls back to the code paint if its texture file is absent - ground upgrades piece by piece like furniture always has.
- paralation-10-stairs-ui: stairsD/stairsU/railing (normal sprites), portraitMatt (48x48 keyed bust; dialogue + stats panel draw it via the portraitImg override, pixel face until then), title (full-frame 960x544 cover-crop into the existing images/title.png hook).
...
### Importer keying, v3 (Aug 16 2026): five stages, and why
Tim reported houses "bleeding into the grass". Root cause: the original single flood fill used threshold 180 vs the corner colour, and the houses' pale tan siding sits only ~130 from the pink backdrop, so the fill poured through outline gaps and ate the WALLS (grass showed through the sprite). The same threshold had also been quietly eating warm-toned furniture parts all along: the desk was missing legs, the mirror frame was slivers, the rugs' borders were ragged. Nobody noticed because there was no pink behind them indoors.
- Stage 1 seed: tight flood (60) from the border, unmistakable backdrop only.
- Stage 2 expand: quadratic fit of the backdrop (vignette included) from seed pixels, grow into anything within 45 of the fit at its own position, OR anything that is a scalar SHADE of the fit (drop shadows; resid<32, scale 0.32-1.25). Connectivity from the border still required, so pillows/mirror glass survive.
- Stage 3 pockets: enclosed regions vs the fit (22), enclosure + 400px minimum area guards (unchanged from v2).
- Stage 4 islands: opaque fragments <250px disconnected from the main silhouette are residue.
- Stage 5 pink fringe: small (<800px) magenta-family components (r>b, b>g+8) touching transparency are backdrop-coloured junk: the generator draws pink grass tufts at house bases despite the "no ground" prompt. The size cap is what protects the duvet and the pink rug, which are magenta-family but huge.
- 'lamp' is in AGGRESSIVE_KEY: its raw lights the backdrop up around the lantern (additive glow, neither near the fit nor a shade of it), so it keys with a wide 170 distance instead. Safe because nothing on a lamp is pink. Add any future glowing asset to that set.
- Every threshold above was chosen by measuring both sides of the boundary, and the regression harness (scratchpad regress.py pattern: old sprite vs new import, opaque-pixel delta per asset) is how changes here get validated. After this rewrite the whole set was re-imported: houses healed +5 to +22%, furniture within ±2% except genuine healings (desk +10%, rugs +30%, mirror +22%, backpack +30%, hoop +29%).

### Comic outline (importer stage 6, Aug 16 2026)
- Tim's call: every sprite gets a uniform near-black contour for total separation from any ground ("comic bookish, but still pixel art"). Applied at IMPORT, not generation: one constant (OUTLINE in import_asset.py, currently 2px = 1 logical pixel), uniform across all assets, changeable by re-running the importer. No ComfyUI workflow changes were needed, and none should be added for outlines: the model cannot hold a line weight consistent across assets, the importer can.
- pixelize now fits the art with an OUTLINE margin and grows the contour OUTWARD (inward would swallow 2px chair legs), using a CROSS structuring element: diamond growth chamfers diagonal steps to 45 degrees. A square kernel grows a knob on every staircase step and reads jagged, which was Tim's complaint about the first outlined build.
- Before outlining, _smooth_silhouette de-jags the sprite edge: transparent 1px notches with 3+ opaque orthogonal neighbours get filled (coloured from their neighbours), opaque 1px nubs with <=1 neighbour get shaved. Two passes. 2px-wide features are untouched.
- Thickness was chosen off a rendered test sheet (0/2/3/4px over game grounds); at 4px small sprites' interior gaps start sealing shut.
- GOTCHA that motivated the cache change below: firebase.json originally served images/ with max-age=86400, so Tim kept seeing day-old sprites after art fixes and reported already-fixed bugs. images/ is now no-cache like the JS. Sprites total ~100KB, the header costs nothing.
